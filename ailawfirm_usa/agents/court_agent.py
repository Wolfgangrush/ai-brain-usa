...
v0.1.1: strips natural-language preamble ("tell me about", "what is", etc.)
        from the payload before passing to india_court_lookup.
...
def handle(payload: str) -> dict:
    ...
    v0.1.1: handles wrappings like "tell me about Bombay HC Aurangabad" →
            extracts "Bombay HC Aurangabad" → fuzzy-matches.
    """
    cleaned = _strip_preamble(payload)
    result = india_court_lookup(cleaned)
    return {
        "agent": "court_agent",
