#!/usr/bin/env python3
"""
onboarding.py — Brain first-run setup.

Asks the user:
  1. How they're using Brain (work / personal / combo)
...
Seeds the entity_registry with confirmed data so Brain knows your world
from minute one — before a single session is indexed.

Usage:
    python3 -m ailawfirm_usa.onboarding
    or: brain init
"""

from pathlib import Path
from ailawfirm_india.entity_registry import EntityRegistry
from ailawfirm_india.entity_detector import detect_entities, scan_for_detection
...
def _ask_mode() -> str:
    _header("Welcome to Brain")
    print("""
  Brain is a personal memory system. To work well, it needs to know
...
def _ask_projects(mode: str) -> list:
    ...
    _hr()
    print("""
  What are your main projects? (These help Brain distinguish project
...
def _warn_ambiguous(people: list) -> list:
    ...
    """
    from ailawfirm_india.entity_registry import COMMON_ENGLISH_WORDS
...
def _generate_alias_bootstrap(
    people: list, projects: list, wings: list, mode: str, config_dir: Path = None
):
    """
    Generate entity-aliasing entity registry + critical facts bootstrap from onboarding data.
    These files teach the AI about the user's world from session one.
    """
    brain_dir = Path(config_dir) if config_dir else Path.home() / ".brain"
    brain_dir.mkdir(parents=True, exist_ok=True)

    # Build entity-aliasing entity codes (first 3 letters of name, uppercase)
    entity_codes = {}
    for p in people:
        name = p["name"]
        code = name[:3].upper()
    ...
    # entity-aliasing entity registry
    registry_lines = [
        "# entity-aliasing Entity Registry",
        "# Auto-generated by brain init. Update as needed.",
        "",
        "## People",
    ]
    for p in people:
    ...
    registry_lines.extend(
        [
            "",
            "## entity-aliasing Quick Reference",
    ...
    (brain_dir / "alias_entities.md").write_text("\n".join(registry_lines))
    ...
    (brain_dir / "critical_facts.md").write_text("\n".join(facts_lines))
...
def run_onboarding(
  ...
  Brain will check the context before treating them as person names.
...
""")
    ...
    # Generate entity-aliasing entity registry + critical facts bootstrap
    _generate_alias_bootstrap(people, projects, wings, mode, config_dir)
    ...
    print("\n  entity-aliasing entity registry: ~/.ailawfirm-usa/alias_entities.md")
    print("  Critical facts bootstrap: ~/.ailawfirm-usa/critical_facts.md")
