#!/usr/bin/env python3
"""
entity_registry.py — Persistent personal entity registry for Brain.
...
def _wikipedia_lookup(word: str) -> dict:
    ...
    """
    try:
        url = f"https://en.wikipedia.org/api/rest_v1/page/summary/{urllib.parse.quote(word, safe='')}"
...
class EntityRegistry:
    ...
    Stored at ~/.ailawfirm-usa/entity_registry.json
    Schema:
    {
    ...
    DEFAULT_PATH = Path.home() / ".brain" / "entity_registry.json"
    ...
    def learn_from_text(self, text: str, min_confidence: float = 0.75) -> list:
        ...
        """
        from ailawfirm_india.entity_detector import (
            extract_candidates,
            score_entity,
            classify_entity,
        )
