...
def test_local_config_wins_even_when_cloud_key_is_present(monkeypatch):
    ...
    import ailawfirm_usa.brain.llm as llm_mod
    ...
    monkeypatch.setattr(llm_mod, "_local_complete", _fake_local)
    monkeypatch.setattr(llm_mod, "_cloud_complete", _fake_cloud)

    assert llm_mod.complete("sys", "user") == "local-answer"
...
def test_force_cloud_env_var_is_the_only_way_to_override_local(monkeypatch):
    ...
    assert llm_mod.complete("sys", "user") == "cloud-answer"
