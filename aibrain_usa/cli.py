...
Commands:
    aibrain_usa reception               Boot the brain, verify specialists, show recap
    aibrain_usa ask "<question>"        One-shot query — routes to the right specialist
    aibrain_usa chat                    Interactive session — every line is routed
    aibrain_usa recap [-n N]            Show recent-session retrospective memory
    aibrain_usa mine <dir> [--mode M]   Mine project files into the palace
    aibrain_usa init <dir>              Detect rooms from folder structure
    aibrain_usa court "scotus"          Look up a US federal court
    aibrain_usa cite "347 U.S. 483"     Validate Bluebook citation
    aibrain_usa calendar add ...        Add ICS calendar entry
    aibrain_usa update [--quiet]        Pull latest firm code from upstream
    aibrain_usa connect-local [--yes]   Install Ollama + Qwen3 + write config
...
def cmd_court(args):
    from aibrain_usa.mcp_tools.usa_court_lookup import usa_court_lookup
    import json
...
def cmd_calendar(args):
    from aibrain_usa.mcp_tools.usa_calendar_sync import usa_calendar_sync
    import json
...
def cmd_ask(args):
    ...
    from aibrain_usa.brain.repl import run_ask
...
def cmd_chat(args):
    ...
    from aibrain_usa.brain.repl import run_chat
...
def cmd_reception(args):
    ...
    from aibrain_usa.brain.reception import run_reception
...
def cmd_recap(args):
    ...
    from aibrain_usa.brain.memory import recap
