...
Reads brain.yaml from the project directory to know the wing + rooms.
...
def load_config(project_dir: str) -> dict:
    """Load brain.yaml from project directory (falls back to mempal.yaml)."""
    import yaml

    config_path = Path(project_dir).expanduser().resolve() / "brain.yaml"
    if not config_path.exists():
        ...
        if legacy_path.exists():
            ...
            print(f"ERROR: No brain.yaml found in {project_dir}")
            print(f"Run: brain init {project_dir}")
            sys.exit(1)
...
def mine(
    ...
    print('\n  Next: brain search "what you\'re looking for"')
    print(f"{'=' * 55}\n")
...
def status(palace_path: str):
    ...
    print(f"\n{'=' * 55}")
    print(f"  Brain Status — {len(metas)} drawers")
    print(f"{'=' * 55}\n")
