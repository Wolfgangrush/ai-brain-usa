...
Usage from CLI:
    ...
    aibrain-usa connect-local --yes      # skip prompts
    aibrain-usa connect-local --model qwen3:7b   # force a specific model

Result: ~/.aibrain-usa/config.json is written with ai_provider=ollama and the
...
COUNTRY = "usa"
CONFIG_DIR = Path.home() / f".aibrain-{COUNTRY}"
...
def install_ollama_instructions() -> None:
    ...
    print()
    print("Then run `aibrain-usa connect-local` again.")
...
def cmd_connect_local(args) -> int:
    print()
    print("=" * 60)
    print(f"  AI Brain — {COUNTRY.upper()} · Local-AI Bridge")
    print("=" * 60)
    print()
    ...
    print("  aibrain-usa connect-cloud --provider deepseek --cloud-warning-acknowledged")
    print()
    return 0
