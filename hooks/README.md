# the local memory layer Hooks — Auto-Save for Terminal AI Tools

These hook scripts make the local memory layer save automatically. No manual "save" commands needed.
...
- **`STATE_DIR`** — Where hook state is stored (defaults to `~/.ailawfirm-usa/hook_state/`)
...
```bash
brain mine <dir>               # Mine all files in a directory
brain mine <dir> --mode convos # Mine conversation transcripts only
```
...
Check the hook log:
```bash
cat ~/.ailawfirm-usa/hook_state/hook.log
```
