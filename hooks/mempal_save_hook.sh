#!/bin/bash
# BRAIN SAVE HOOK — Auto-save every N exchanges
#
...
#
# === BRAIN CLI ===
# This repo uses: brain mine <dir>
# or:            brain mine <dir> --mode convos
# Set BRAIN_DIR below if you want the hook to auto-ingest after blocking.
# Leave blank to rely on the AI's own save instructions.
#
# === CONFIGURATION ===

SAVE_INTERVAL=15  # Save every N human messages (adjust to taste)
STATE_DIR="$HOME/.brain/hook_state"
mkdir -p "$STATE_DIR"

# Optional: set to the directory you want auto-ingested on each save trigger.
# Example: BRAIN_DIR="$HOME/conversations"
...
# Log for debugging (check ~/.ailawfirm-usa/hook_state/hook.log)
...
if [ "$SINCE_LAST" -ge "$SAVE_INTERVAL" ] && [ "$EXCHANGE_COUNT" -gt 0 ]; then
    ...
    # Optional: run brain ingest in background if BRAIN_DIR is set
    if [ -n "$BRAIN_DIR" ] && [ -d "$BRAIN_DIR" ]; then
        SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
        REPO_DIR="$(dirname "$SCRIPT_DIR")"
        python3 -m brain mine "$BRAIN_DIR" >> "$STATE_DIR/hook.log" 2>&1 &
    fi
